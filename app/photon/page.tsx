...
export default function PhotonPage() {
  ...
  const handleToggle = async () => {
    ...
    if (result.includes("ONAYLANDI")) { setIsDark(!isDark); }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          <div className="border-[5px] border-black bg-white w-full lg:w-[600px] brutal-shadow-lg">
            <div className="border-b-[3px] border-black px-4 py-2 bg-[#ffcc00] text-black font-mono text-[10px] uppercase">
              PHOTON.EXE
            </div>
            <div className="p-6">
              <h2 className="brutal-heading text-4xl mb-6">{t("photon.title")}</h2>
              <div className="flex items-center justify-center h-44 mb-6">
                <div className={`w-28 h-28 border-[5px] border-black transition-all duration-500 ${isDark ? "bg-white shadow-[8px_8px_0px_#ffcc00]" : "bg-[#ffcc00] shadow-[8px_8px_0px_#000000]"}`} />
              </div>
              <button onClick={handleToggle} className="brutal-btn brutal-btn-yellow w-full">
