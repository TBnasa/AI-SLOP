...
export default function CalcPage() {
  ...
  const handleNumber = (num: string) => {
    if (waitingForSecondNumber) { setDisplay(num); setWaitingForSecondNumber(false); }
    else { setDisplay(display === "0" ? num : display + num); }
  };

  const handleOperator = (nextOperator: string) => {
    const inputValue = parseFloat(display);
    if (firstNumber === null) { setFirstNumber(inputValue); }
    else if (operator) { const result = performCalculation[operator](firstNumber, inputValue); setDisplay(String(result)); setFirstNumber(result); }
    setWaitingForSecondNumber(true); setOperator(nextOperator);
  };
  ...
  const calculate = async () => {
    ...
    if (operator && firstNumber !== null) {
      ...
      setDisplay(String(result)); setFirstNumber(null); setOperator(null); setWaitingForSecondNumber(false);
      setLoading(true);
      const slopText = await executeSlop(t("calc.title"), t("calc.prompt", fullOp, result));
      setSlop(slopText); setLoading(false);
    }
  };

  const clear = () => { setDisplay("0"); setFirstNumber(null); setOperator(null); setWaitingForSecondNumber(false); setSlop(""); };

  const btn = "font-display text-2xl py-3 border-[3px] border-black bg-white text-black hover:bg-black hover:text-white transition-all cursor-pointer";
  const opBtn = `${btn} bg-black text-white hover:bg-white hover:text-black`;
  const eqBtn = `${btn} bg-[#00cc00] text-black row-span-2 hover:bg-[#009900]`;
