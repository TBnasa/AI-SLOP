      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          <div className="border-[5px] border-black bg-white w-full lg:w-[600px] brutal-shadow-lg">
            <div className="border-b-[3px] border-black px-4 py-2 bg-[#00cc00] text-black font-mono text-[10px] uppercase">
              CONVERTER.EXE
            </div>
            <div className="p-6">
              ...
              <div className="flex gap-4 mb-5">
                <div className="flex-grow">
                  <label className="block font-mono text-[10px] uppercase tracking-widest mb-2">{t("converter.labelFrom")}</label>
                  <input type="number" value={meters} onChange={(e) => setMeters(e.target.value)} className="brutal-input w-full text-sm" placeholder={t("converter.placeholder")} />
                </div>
                <div className="flex items-end pb-4 font-display text-3xl">→</div>
                <div className="flex-grow">
                  <label className="block font-mono text-[10px] uppercase tracking-widest mb-2">{t("converter.labelTo")}</label>
                  <div className="brutal-input w-full h-[52px] flex items-center bg-gray-100 text-gray-400 text-sm">{meters ? parseFloat(meters) * 100 : "?"}</div>
                </div>
              </div>
              <button onClick={handleConvert} className="brutal-btn brutal-btn-green w-full">{t("converter.button")}</button>
            </div>
          </div>
